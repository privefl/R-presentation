...
* In C++, vector indices start at 0. I'll say this again because it's so 
  important: <mark>__IN C++, VECTOR INDICES START AT 0__</mark>! This is a very common 
  source of bugs when converting R functions to C++.

* Use `=` for assignment, not `<-`. C++ provides operators that modify in-place: `total += x[i]` is equivalent to 
  <mark>`total = total + x[i]`</mark>. Similar in-place operators are `-=`, `*=`, and `/=`.
