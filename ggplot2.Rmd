...
### geom_smooth inherits from top 'aes'
...
### geom_smooth on all (without grouping)
...
```{r}
str(res.df <- readRDS("results.rds"), 
    width = 65, strict.width = "wrap")
```

```{r, results='hide'}
library(ggplot2)
p <- ggplot(subset(res.df, Dp %in% c(0.05, 0.1)),
            aes(NumberOfTopHits, Power, color = Method)) +
  geom_line() +
...
```{r out.width = "100%", fig.asp = 0.65, echo=FALSE}
p
```
...
# Go read [chapter 3 of *R for Data Science*](http://r4ds.had.co.nz/data-visualisation.html)
...
# Go to [Stack Overflow ](https://stackoverflow.com/questions/tagged/ggplot2?sort=faq) for common questions
...
library(ggplot2) ## bug, why ylim() is not found?
```


```{r out.width = "80%", fig.asp = 0.7, cache=FALSE}
plot(test) + ylim(0.5, NA)
```
...
for (i in seq_along(var)) {
  p_list[[i]] <- ggplot(iris) + geom_bar(aes_string(var[i]))
}
p_list[[2]]
```
